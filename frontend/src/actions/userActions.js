...
export const updatePassword = (passwords) => async (dispatch) => {
  const passes = JSON.stringify(passwords);
  try {
    ...
    const { data } = await axios.put("/api/v1/password/update", passes, config);
    dispatch({
...
export const updateUser = (id, userData) => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.put(`/api/v1/admin/user/${id}`, json, config);
